    ...
    return class Adapter extends Base {
        ...
        async getDefaultBridgeAddresses() {
            ...
            if (!addresses.l1Nullifier) {
                ...
                this._providerL2()._setL1NullifierAndNativeTokenVault(l1Nullifier, l1NativeTokenVault);
            }
            else {
        ...
        async getNativeTokenVaultL1() {
            // FIXME: maybe makes sense to provide an API to do it in one call
            const bridgeContracts = await this.getDefaultBridgeAddresses();
            const sharedBridge = bridgeContracts.sharedL1;
            const l1AssetRouter = IL1AssetRouter__factory.connect(sharedBridge, this._providerL1());
            const l1NtvAddress = await l1AssetRouter.nativeTokenVault();
            return IL1NativeTokenVault__factory.connect(l1NtvAddress, this._providerL1());
        }
        /**
        ...
        async _depositNonBaseTokenToNonETHBasedChain(transaction) {
            ...
            return await this.getPriorityOpResponse(await this._signerL1().sendTransaction(tx));
        }
        ...
        async _getL2GasLimit(transaction) {
            if (transaction.bridgeAddress) {
                ...
                return await this._providerL2().estimateDefaultBridgeDepositL2Gas(this._providerL1(), transaction.token, transaction.amount, transaction.to, await this.getAddress(), transaction.gasPerPubdataByte);
            }
        }
         ...
         * @throws {Error} If log proof can not be found.
         */
        async getFinalizeWithdrawalParams(withdrawalHash, index = 0, interopMode) {
            const { log, l1BatchTxId } = await this._getWithdrawalLog(withdrawalHash, index);
            const { l2ToL1LogIndex } = await this._getWithdrawalL2ToL1Log(withdrawalHash, index);
            const sender = ethers.dataSlice(log.topics[1], 12);
            const proof = await this._providerL2().getLogProof(withdrawalHash, l2ToL1LogIndex, interopMode);
            if (!proof) {
                throw new Error('Log proof not found!');
            }
            const message = ethers.AbiCoder.defaultAbiCoder().decode(['bytes'], log.data)[0];
        ...
        async getL1NullifierAddress() {
            const addresses = await this.getDefaultBridgeAddresses();
            return await IL1AssetRouter__factory.connect(addresses.sharedL1, this._signerL1()).L1_NULLIFIER();
        }
        /**
        ...
        async finalizeWithdrawal(withdrawalHash, index = 0, overrides) {
            const finalizeWithdrawalParams = await this.getFinalizeWithdrawalParams(withdrawalHash, index);
        ...
        async claimFailedDeposit(depositHash, overrides) {
            ...
            const successL2ToL1LogIndex = receipt.l2ToL1Logs.findIndex(l2ToL1log => isAddressEq(l2ToL1log.sender, BOOTLOADER_FORMAL_ADDRESS) &&
            ...
            let assetId;
            let assetData;
            const l1AssetRouter = IL1AssetRouter__factory.connect(l1BridgeAddress, this._signerL1());
            ...
            try {
                ...
                assetData = encodeNativeTokenVaultTransferData(calldata['_amount'], calldata['_l2Receiver'], calldata['_l1Token']);
                ...
                const calldata = l2ARInterface.decodeFunctionData('finalizeDeposit', tx.data);
                ...
                const transferDataDecoded = ethers.AbiCoder.defaultAbiCoder().decode(['address', 'address', 'address', 'uint256', 'bytes'], transferData);
                assetData = ethers.AbiCoder.defaultAbiCoder().encode(['uint256', 'address', 'address'], [transferDataDecoded[3], transferDataDecoded[1], l1TokenAddress]);
            ...
            const bridgeRecoverFailedTransfer = l1AssetRouter['bridgeRecoverFailedTransfer(uint256,address,bytes32,bytes,bytes32,uint256,uint256,uint16,bytes32[])'];
            return await bridgeRecoverFailedTransfer((await this._providerL2().getNetwork()).chainId, depositSender, assetId, assetData, depositHash, receipt.l1BatchNumber, proof.id, receipt.l1BatchTxIndex, proof.proof, overrides ?? {});
        }
        /**
        ...
        async requestExecute(transaction) {
            ...
            return this.getPriorityOpResponse(await this._signerL1().sendTransaction(requestExecuteTx));
        }
        /**
        ...
        async getRequestExecuteAllowanceParams(transaction) {
            const bridgehub = await this.getBridgehubContract();
            const chainId = (await this._providerL2().getNetwork()).chainId;
            const isETHBaseToken = isAddressEq(await bridgehub.baseToken(chainId), ETH_ADDRESS_IN_CONTRACTS);
            if (isETHBaseToken) {
                throw new Error("ETH token can't be approved! The address of the token does not exist on L1.");
            }
            const { ...tx } = transaction;
...
export function AdapterL2(Base) {
    return class Adapter extends Base {
        /**
         * Returns a provider instance for connecting to an L2 network.
         */
        ...
        async getL2BridgeContracts() {
            ...
            return {
                ...
                shared: IL2SharedBridge__factory.connect(addresses.sharedL2, this._signerL2()),
            };
        }
